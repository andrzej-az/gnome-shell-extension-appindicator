...
export class AppIndicator extends Signals.EventEmitter {
    ...
    get friendlyTitle() {
        ...
        const getCleanName = name => {
            ...
            // For Flatpak IDs like com.slack.Slack, we want the last part
            let basename = name.split('/').pop();
            if (basename.includes('.') && !basename.includes(' ')) {
                const parts = basename.split('.');
                const lastPart = parts.pop();
                // If the last part is something generic like "Slack", "Client", or the app name, use it
                if (lastPart && lastPart.length > 2 && !genericProcessNames.includes(lastPart.toLowerCase()))
                    basename = lastPart;
                else if (parts.length > 0)
                    basename = parts.pop(); // Fallback to middle part if last is too generic
            } else {
                basename = basename.split('.')[0];
            }

            if (genericProcessNames.some(g => basename.toLowerCase().includes(g)))
                return null;
            if (/^[0-9_-]+$/.test(basename))
                return null;
        ...
        if (this._commandLine) {
            const args = this._commandLine.trim().split(' ');
            for (const arg of args) {
                const name = getCleanName(arg);
                if (name)
                    return name;
            }
        }
        ...
        // 2. Try icon name
        const iconName = this._proxy.IconName;
        const cleanIconName = getCleanName(iconName);
        if (cleanIconName)
            return cleanIconName;

        return title || this.id || 'Unknown';
    }

    get id() {
        return this._proxy ? this._proxy.Id : null;
    }

    get stableId() {
        const id = this.id;
        if (id) {
            // We include the uniqueId to avoid collisions between different apps
            // that might use the same generic ID (like 'chrome_status_icon_1')
